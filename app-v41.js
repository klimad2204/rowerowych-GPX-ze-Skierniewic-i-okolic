...
async function load(){
 initMap();
 try{
  const [cat,geo,att]=await Promise.all([fetch('routes_catalog.json?v=41').then(r=>r.json()),fetch('routes.geojson?v=41').then(r=>r.json()),fetch('attractions.json?v=41').then(r=>r.json())]);
...
function renderStats(){
 ...
 stats.innerHTML=`<div class="row"><div><b>${routes.length}</b><br><span class="mini">tras razem</span></div><div><b>${Math.round(total)} km</b><br><span class="mini">suma katalogu</span></div><div><b>${imp}</b><br><span class="mini">import GPX</span></div></div><p class="mini">Krótkie: ${krótkie}, średnie: ${sred}, długie: ${dl}.</p>`;
}
function render(){
...
function parseGpx(text,name){
 ...
 return {type:'Feature',properties:{id,name:name.replace(/\.gpx$/i,'')||'Import GPX',distance_km:+d.toFixed(1),points_count:pts.length,type,imported:true},geometry:{type:'LineString',coordinates:simp}};
}
...
function bind(){
 ...
 if('serviceWorker' in navigator) navigator.serviceWorker.register('sw-v41.js?v=41').catch(()=>{});
}
