...
async function load(){
 initMap();
 try{
  const [cat,geo,att]=await Promise.all([fetch('routes_catalog.json?v=50').then(r=>r.json()),fetch('routes.geojson?v=50').then(r=>r.json()),fetch('attractions.json?v=50').then(r=>r.json())]);
  ...
  msg(`Wczytano ${routes.length} tras. V5.0: naprawione wytyczanie, punkty START/PRZEZ/META i atrakcje przy trasie.`);
...
function makePlan(){
 const dist=$('planDist').value; const names=getPlanNames(); const text=names.join(' → ');
 ...
 if(planned.feature){
   ...
   drawPlanMarkers(names, planned.feature.geometry.coordinates);
   ...
   renderRouteAttractions(planned.feature.geometry.coordinates,'Ciekawostki, pomniki, cmentarze wojenne i zabytki przy planowanej trasie');
   const warnings=logicWarnings(names, planned.feature.geometry.coordinates);
   warnHtml=warnings.length?`<div class="warnbox"><b>Uwaga:</b><br>${warnings.map(esc).join('<br>')}</div>`:'<div class="okbox">Kolejność wygląda logicznie: Start → Przez → Meta.</div>';
 }
 $('planOut').classList.remove('hidden'); $('planOut').innerHTML=`<h3>Szkic trasy ${esc(dist)}</h3><p><b>Kolejność:</b></p><p>${esc(text)}</p>${warnHtml}<p class="status">${esc(mapMsg)}</p>${planned.missing.length?`<p class="mini">Nie rozpoznano miejsc: ${esc(planned.missing.join(', '))}. Te punkty nie zostały narysowane na mapie.</p>`:''}<div class="grid"><button id="copyPlan">Kopiuj kolejność</button><a class="btn primary" target="_blank" href="${routeUrl}">Wytycz dokładnie w Mapy.com</a>${planned.feature?'<button id="openMapyNav">Otwórz nawigację Mapy.com</button><button id="downloadPlanGpx">Pobierz GPX planu</button>':''}</div><p class="mini">Na mapie aplikacji widzisz szkic po punktach. Dokładną trasę po drogach wytycza Mapy.com po kliknięciu przycisku. Meta zawsze jest ostatnia.</p>`;
 setTimeout(()=>{
...
function downloadBackup(){
 const data={version:'5.0',created:new Date().toISOString(),routes,features,imported:getImported()};
...
function bind(){
 ...
 document.addEventListener('click',e=>{ const show=e.target.closest('[data-show]'); if(show) selectRoute(show.dataset.show); const det=e.target.closest('[data-details]'); if(det) selectRoute(det.dataset.details); const near=e.target.closest('[data-near]'); if(near) showNearby(Number(near.dataset.near)); const cp=e.target.closest('[data-copy-poi]'); if(cp) navigator.clipboard?.writeText(cp.dataset.copyPoi).then(()=>alert('Skopiowano GPS.')).catch(()=>alert(cp.dataset.copyPoi)); });
 if('serviceWorker' in navigator) navigator.serviceWorker.register('sw-v50.js?v=50').catch(()=>{});
}
