...
async function load(){
 initMap();
 try{
  const [cat,geo,att]=await Promise.all([fetch('routes_catalog.json?v=62').then(r=>r.json()),fetch('routes.geojson?v=62').then(r=>r.json()),fetch('attractions.json?v=62').then(r=>r.json())]);
  ...
  msg(`Wczytano ${routes.length} tras. V6.3: naprawiono przycisk szkicu trasy i dodano widoczną diagnostykę kliknięcia.`);
...
async function findPlaceCoordAsync(name){
 const direct=findPlaceCoord(name);
 if(direct) return direct;
 const key=normalizePlace(name);
 if(!key) return null;
 if(geocodeCache[key]) return geocodeCache[key];
 try{
   msg('Szukam punktu online: '+name+'...');
   const ctrl=new AbortController();
   const timer=setTimeout(()=>ctrl.abort(),4500);
   const url='https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=pl&q='+encodeURIComponent(name+', Polska');
   const res=await fetch(url,{headers:{'Accept':'application/json'}, signal:ctrl.signal});
   clearTimeout(timer);
   if(!res.ok) throw new Error('geocode');
   const arr=await res.json();
   if(arr && arr[0]){
     const c=[Number(arr[0].lon), Number(arr[0].lat), arr[0].display_name || name];
     geocodeCache[key]=c;
     return c;
   }
 }catch(e){ console.warn('Nie udało się znaleźć punktu online:', name, e); }
 return null;
}
...
async function planFeatureFromNames(names){
 const coords=[], missing=[], labels=[];
 for(const name of names){
   const c=await findPlaceCoordAsync(name);
   if(c){ coords.push([c[0],c[1]]); labels.push(name); }
   else missing.push(name);
 }
 if(coords.length<2) return {feature:null, missing, labels};
 return {feature:{type:'Feature',properties:{id:'planned_'+Date.now(),name:'Planowana trasa',distance_km:+distCoords(coords).toFixed(1),type:'plan'},geometry:{type:'LineString',coordinates:coords}}, missing, labels};
}
...
async function makePlan(){
 ...
 if(names.length<2){ alert('Wpisz co najmniej Start i Meta.'); return; }
 msg('Klik działa. Tworzę szkic trasy: '+names.join(' → '));
 const plannedBase=await planFeatureFromNames(names);
 ...
 }catch(err){ console.error(err); msg('Błąd szkicu trasy: '+(err && err.message ? err.message : err)); alert('Błąd szkicu trasy. Odśwież stronę i sprawdź, czy wgrano index.html oraz app-v63.js.'); }
}
...
function windAdvice(){
 ...
 const names=getPlanNames(); const coords=names.map(findPlaceCoord).filter(Boolean).map(c=>[c[0],c[1]]); const base={feature: coords.length>=2 ? {geometry:{coordinates:coords}} : null};
 if(!base.feature){ out.textContent='Najpierw wpisz Start, Meta i ewentualne Przez, potem utwórz szkic trasy.'; return; }
 const planCoords=base.feature.geometry.coordinates; const s=planCoords[0], e=planCoords[planCoords.length-1];
...
function downloadBackup(){
 const data={version:'6.3',created:new Date().toISOString(),routes,features,imported:getImported()};
...
function bind(){
 ...
 $('search').oninput=render; $('filter').onchange=render; $('btnImport').onclick=importFiles; initPlanner(); const bp=$('btnPlan'); if(bp){ bp.onclick=(ev)=>{ev.preventDefault(); makePlan();}; bp.addEventListener('touchend',(ev)=>{ev.preventDefault(); makePlan();},{passive:false}); }
 document.addEventListener('click',e=>{ if(e.target && e.target.id==='btnPlan'){ e.preventDefault(); makePlan(); return; } const show=e.target.closest('[data-show]'); if(show) selectRoute(show.dataset.show); const det=e.target.closest('[data-details]'); if(det) selectRoute(det.dataset.details); const near=e.target.closest('[data-near]'); if(near) showNearby(Number(near.dataset.near)); const cp=e.target.closest('[data-copy-poi]'); if(cp) navigator.clipboard?.writeText(cp.dataset.copyPoi).then(()=>alert('Skopiowano GPS.')).catch(()=>alert(cp.dataset.copyPoi)); const au=e.target.closest('[data-add-unvisited]'); if(au){addViaValue(au.dataset.addUnvisited); document.getElementById('planStart')?.scrollIntoView({behavior:'smooth'});} });
 if('serviceWorker' in navigator) navigator.serviceWorker.register('sw-v63.js?v=63').catch(()=>{});
}
window.testPlan=makePlan; window.addEventListener('DOMContentLoaded',()=>{bind(); load();});
})();
