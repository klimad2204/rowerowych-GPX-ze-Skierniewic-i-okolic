...
async function load(){
 initMap();
 try{
  const [cat,geo,att]=await Promise.all([fetch('routes_catalog.json?v=46').then(r=>r.json()),fetch('routes.geojson?v=46').then(r=>r.json()),fetch('attractions.json?v=46').then(r=>r.json())]);
  ...
  msg(`Wczytano ${routes.length} tras. V4.6: naprawiono statystyki i przycisk Wytycz w Mapy.cz.`);
...
function downloadBackup(){
 const data={version:'4.6',created:new Date().toISOString(),routes,features,imported:getImported()};
...
function bind(){
 ...
 if('serviceWorker' in navigator) navigator.serviceWorker.register('sw-v45.js?v=46').catch(()=>{});
}
